...
def merge_group(G, G_min, src, quality_list, all_group_result, weight):
    ...
    while is_finish == False:
        ...
        for i in range(len(sort_group)):
            for j in range(i+1, len(sort_group)):
                ...
                # Calculate merging group cost
                tmp_path_info_merge = add_merge_info(G_merge, src, merge_group_info, i, j, quality_list)
                
                G_tmp_group = copy.deepcopy(G)
                for m in range(len(tmp_path_info_merge)):
                    if tmp_path_info_merge[m][0] == []: continue
                    G_tmp_group = update_graph(G_tmp_group, tmp_path_info_merge[m])
                ...
                G_tmp_path = copy.deepcopy(G)
                for m in range(len(new_path_info_merge)):
                    if new_path_info_merge[m][0] == []: continue
                    G_tmp_path = update_graph(G_tmp_path, new_path_info_merge[m])

                merge_cost_paths = Graph.cal_total_cost(G_tmp_path, merge_weight, True)

                cost_metric[(i,j)] = [round(orig_cost_groups[0] - merge_cost_paths[0], 2), new_path_info_merge, G_tmp_path]
    ...
    merge_final_info = copy.deepcopy(merge_group_info)
    # for i in range(len(merge_final_info)):
    #     merge_final_info = merge_path(G, G_merge, src, quality_list, merge_final_info, i, weight)

    G_final = copy.deepcopy(G)
    for m in range(len(merge_final_info)):
        if merge_final_info[m][0] == []: continue
        G_final = update_graph(G_final, merge_final_info[m])

    count_vnf = 0
    delay = [0]
    count_group = 0
    for i in range(len(merge_final_info)):
        count_vnf += Graph.count_vnf(merge_final_info[i][1])
        delay.append(Graph.max_len(merge_final_info[i][0]))
        count_group += 1
