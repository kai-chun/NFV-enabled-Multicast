...
def printGraph(G, pos, filename, service, weight):
    ...
    if filename in ['tmp', 'G_min_miss_vnf', 'path_miss_vnf', 'G_min_remove', 'path_remove', 'G_min_update', 'path_update', 'G_final_0', 'path_final_0', 'G_final_1', 'path_final_1','G_final_2', 'path_final_2']: 
...
find common path
'''
def find_common_path_len(path, dst, shortest_path_set, data_rate, video_type):
...
def add_new_edge(G, path, path_set, dst, e, data_rate, video_type):
    ...
    if path.has_edge(*e) == False:
        ...
        commom_len = find_common_path_len(path, dst, path_set, data_rate, video_type)
