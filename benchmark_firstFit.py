        ...
        for i in range(len(shortest_path)-1):
            ...
            if shortest_path[i] != src and shortest_path[i] != dst:
                ...
                while index_sfc[dst]['index'] < len(sfc[dst])-1:
                    ...
                    if G_min.nodes[j]['capacity'] > data_rate[dst][-1][2]:
                        ...
                        G_min.nodes[j]['capacity'] -= data_rate[dst][-1][2]
            ...
            Graph.add_new_edge(G_min, multicast_path_min, shortest_path_set, dst, e, data_rate[dst][-1], video_type)

    '''
    A corrective subroutine that places the missing NF instances 
    on the closest NFV node from the multicast topology.
    '''
    tmp = benchmark_JPR.update_path(sfc, index_sfc, sort_dsts, G, G_min, multicast_path_min, shortest_path_set, data_rate, video_type)
    ...
    for d in sort_dsts:
        ...
        while index_sfc[dst]['index'] < len(sfc[dst])-1:
            ...
            for i in alternate_nodes:
                ...
                while G_min.nodes[i]['capacity'] > data_rate[dst][-1][2]:
                    ...
                    if find_distance == 1:
                        ...
                        Graph.add_new_edge(G_min, multicast_path_min, update_shortest_path_set, dst, e, data_rate[dst][-1], video_type)
                    else:
                    ...
                    G_min.nodes[i]['capacity'] -= data_rate[dst][-1][2]                    
