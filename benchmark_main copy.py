...
def main(topology, bandwidth, exp_num, group, gen_new_data):
    ...
    while exp_factor <= bound_factor: 
        ...
        for i in range(exp_num):
            ...
            total_delay_unicast += max(delay_unicast)
            total_delay_JPR += max(delay_JPR)
            total_delay_JPR_notReuse += max(delay_JPR_notReuse)
            total_delay_firstFit += max(delay_unicast)
            total_delay_main += max(delay_main)
        ...
        ### Total cost (avg to exp times)
        exp_data_unicast[0].append(total_cost_unicast[0]/exp_num)
        exp_data_JPR[0].append(total_cost_JPR[0]/exp_num)
        exp_data_JPR_notReuse[0].append(total_cost_JPR_notReuse[0]/exp_num)
        exp_data_firstFit[0].append(total_cost_firstFit[0]/exp_num)
        exp_data_main[0].append(total_cost_main[0]/exp_num)
        exp_data_merge[0].append(total_cost_merge[0]/exp_num)

        ### Trans_cost, Proc_cost, Plac_cost, Avg cost, Transcoder Number (avg to user nums)
        total_dst = dst_num * exp_num
        if total_dst == failed_num_unicast:
            for j in range(1,6):
                exp_data_unicast[j].append(0)
        else:
            dst_num_unicast = total_dst - failed_num_unicast
            for j in range(1,4):
                exp_data_unicast[j].append(total_cost_unicast[j]/dst_num_unicast)
            exp_data_unicast[4].append(total_cost_unicast[0]/dst_num_unicast)
            exp_data_unicast[5].append(transcoder_num_unicast/dst_num_unicast)
        if total_dst == failed_num_JPR:
            for j in range(1,6):
                exp_data_JPR[j].append(0)
        else:
            dst_num_JPR = total_dst - failed_num_JPR
            for j in range(1,4):
                exp_data_JPR[j].append(total_cost_JPR[j]/dst_num_JPR)
            exp_data_JPR[4].append(total_cost_JPR[0]/dst_num_JPR)
            exp_data_JPR[5].append(transcoder_num_JPR/dst_num_JPR)
        if total_dst == failed_num_JPR_notReuse:
            for j in range(1,6):
                exp_data_JPR_notReuse[j].append(0)
        else:
            dst_num_JPR_notReuse = total_dst - failed_num_JPR_notReuse
            for j in range(1,4):
                exp_data_JPR_notReuse[j].append(total_cost_JPR_notReuse[j]/dst_num_JPR_notReuse)
            exp_data_JPR_notReuse[4].append(total_cost_JPR_notReuse[0]/dst_num_JPR_notReuse)
            exp_data_JPR_notReuse[5].append(transcoder_num_JPR_notReuse/dst_num_JPR_notReuse)
        if total_dst == failed_num_firstFit:
            for j in range(1,6):
                exp_data_firstFit[j].append(0)
        else:
            dst_num_firstFit = total_dst - failed_num_firstFit
            for j in range(1,4):
                exp_data_firstFit[j].append(total_cost_firstFit[j]/dst_num_firstFit)
            exp_data_firstFit[4].append(total_cost_firstFit[0]/dst_num_firstFit)
            exp_data_firstFit[5].append(transcoder_num_firstFit/dst_num_firstFit)
        if total_dst == failed_num_main:
            for j in range(1,6):
                exp_data_main[j].append(0)
        else:
            dst_num_main = total_dst - failed_num_main
            for j in range(1,4):
                exp_data_main[j].append(total_cost_main[j]/dst_num_main)
            exp_data_main[4].append(total_cost_main[0]/dst_num_main)
            exp_data_main[5].append(transcoder_num_main/dst_num_main)
        if total_dst == failed_num_merge:
            for j in range(1,6):
                exp_data_merge[j].append(0)
        else:
            dst_num_merge = total_dst - failed_num_merge
            for j in range(1,4):
                exp_data_merge[j].append(total_cost_merge[j]/dst_num_merge)
            exp_data_merge[4].append(total_cost_merge[0]/dst_num_merge)
            exp_data_merge[5].append(transcoder_num_merge/dst_num_merge)
        ...
        failed_data_merge.append(failed_num_merge/total_dst)
    ...
    if is_group == 1:
        ...
        plt.tight_layout() 
        plt.savefig('exp_img/'+factor+'/failed_dst_num_'+str(exp_num)+'_k.png')
    else:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = 1')
        plt.tight_layout()
        plt.savefig('exp_img/'+factor+'/failed_dst_num_'+str(exp_num)+'.png')
    ...
    if is_group == 1:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = n')
        plt.tight_layout()
        plt.savefig('exp_img/'+factor+'/total_cost_'+str(exp_num)+'_k.png')
    else:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = 1')
        plt.tight_layout()
        plt.savefig('exp_img/'+factor+'/total_cost_'+str(exp_num)+'.png')
    ...
    if is_group == 1:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = n')
        plt.tight_layout() 
        plt.savefig('exp_img/'+factor+'/avg_cost_'+str(exp_num)+'_k.png')
    else:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = 1')
        plt.tight_layout()
        plt.savefig('exp_img/'+factor+'/avg_cost_'+str(exp_num)+'.png')
    ...
    if is_group == 1:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = n')
        plt.tight_layout() 
        plt.savefig('exp_img/'+factor+'/tran_cost_'+str(exp_num)+'_k.png')
    else:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = 1')
        plt.tight_layout()
        plt.savefig('exp_img/'+factor+'/tran_cost_'+str(exp_num)+'.png')
    ...
    if is_group == 1:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = n')
        plt.tight_layout() 
        plt.savefig('exp_img/'+factor+'/proc_cost_'+str(exp_num)+'_k.png')
    else:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = 1')
        plt.tight_layout()
        plt.savefig('exp_img/'+factor+'/proc_cost_'+str(exp_num)+'.png')
    ...
    if is_group == 1:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = n')
        plt.tight_layout() 
        plt.savefig('exp_img/'+factor+'/plac_cost_'+str(exp_num)+'_k.png')
    else:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = 1')
        plt.tight_layout()
        plt.savefig('exp_img/'+factor+'/plac_cost_'+str(exp_num)+'.png')
    ...
    if is_group == 1:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = n')
        plt.tight_layout() 
        plt.savefig('exp_img/'+factor+'/transcoder_num_'+str(exp_num)+'_k.png')
    else:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = 1')
        plt.tight_layout()
        plt.savefig('exp_img/'+factor+'/transcoder_num_'+str(exp_num)+'.png')
    ...
    if is_group == 1:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = n')
        plt.tight_layout() 
        plt.savefig('exp_img/'+factor+'/delay_'+str(exp_num)+'_k.png')
    else:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = 1')
        plt.tight_layout()
        plt.savefig('exp_img/'+factor+'/delay_'+str(exp_num)+'.png')
    ...
    if is_group == 1:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = n')
        plt.tight_layout() 
        plt.savefig('exp_img/'+factor+'/running_time_'+str(exp_num)+'_k.png')
    else:
        plt.title('Graph ('+str(len(input_graph[0].nodes))+', '+str(len(input_graph[0].edges))+'), k = 1')
        plt.tight_layout()
        plt.savefig('exp_img/'+factor+'/running_time_'+str(exp_num)+'.png')
    
    plt.close()

    title = {0: "Total_cost", 1: "Trans_cost", 2: "Proc_cost cost", 3: "Plac_cost", \
        4: "Avg_cost", 5: "Transcoder_num", 6: "Delay", 7: "Running_time"}
    
    with pd.ExcelWriter('exp_data/'+factor+'_bw'+str(bandwidth)+'_exp'+str(exp_num)+'_g'+str(is_group)+'.xlsx') as writer:  
        for i in range(8):
            data = pd.DataFrame({"exp times":times,"Unicast":exp_data_unicast[i],"JPR":exp_data_JPR[i],\
